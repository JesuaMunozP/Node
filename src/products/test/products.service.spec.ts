...
describe('ProductService', () => {
  ...
  const mockProductRepository = {
    ...
    findOneBy: jest
      .fn()
      .mockImplementation((product) => Promise.resolve({ id: Date.now() })),
    findOne: jest
      .fn()
      .mockImplementation((product) => Promise.resolve({ id: Date.now() })),
    update: jest.fn(),
  ...
  it('Should update a user', async () => {
    ...
    // const resultProduct = await service.update(
    //   '033ae036-6a20-4760-a11c-f83667474085',
    //   {
    //     ...product,
    //     title: 'titleUpdate',
    //   },
  ...
  it('should expect an error trying to update a product', async () => {
    await service
      .update('033ae036-6a20-4760-a11c-f83667474085', { title: 'Test' })
      .catch((e) => {
        expect(e);
      });
  });
